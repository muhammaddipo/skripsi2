...
\newcommand{\tanggal}{28/05/2020} %Tanggal pembuatan dokumen
...
\textbf{Gambar 22} menunjukan jika \textit{keyword} pada kalimat berawalan \textbf{Then} nya yaitu \textbf{tabel}. untuk \textit{keyword} \textbf{tabel}, cara pengerjaannya hampir mirip dengan \textit{keyword} \textbf{halaman}, tapi untuk \textit{keyword} \textbf{tabel}, harus menjalankan \textbf{query} terlebih dahulu untuk mendapatkan hasilnya. Setelah \textit{method} contoh \textit{addAdmin} dijalankan, maka seharusnya sudah ada admin baru terdaftar pada \textit{database}, maka dari itu menjalankan \textbf{query} dengan memeriksa apakah \textit{username} yang tadi didaftarkan menggunakan \textit{method addAdmin} sudah berhasil tercatat pada \textit{database}, jika sudah maka dengan metode \textbf{Count} yang terlihat pada baris 6 gambar 22, keluaranya harus nya 1. \textbf{Actual result} nya disini yaitu berupa \textbf{query} yang menghasilkan suatu keluaran dari \textit{database}, dan \textbf{expected result} nya yaitu angka 1, karena jika sistem berhasil melakukan \textbf{query} tersebut, maka seharusnya admin sudah terdaftar dan tercatat pada \textit{database} maka angka 1 melambangkan bahwa ada 1 \textit{username} dengan nama tersebut yang sudah didaftarkan sebelumnya tercatat pada \textit{database}. Dengan menggunakan metode \textbf{assertEquals} akan diperiksa apakah sama value dari kedua variabel tersebut, jika sama maka \textit{unit testing} dinyatakan berhasil, jika tidak maka akan muncul  \textit{error} sebagai tanda bahwa \textit{unit testing} tersebut gagal.

		\item \textbf{Menulis dokumen skripsi.}\\ 
		{\bf Status :} Ada sejak rencana kerja skripsi.\\
		{\bf Hasil :} Menulis dokumen skripsi sudah dilakukan untuk Bab 1, Bab 2, Bab 3, Bab 4, dan Bab 5.
